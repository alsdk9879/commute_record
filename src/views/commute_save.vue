...
const masterStartTime = {
  ...
  maxTime: `${currentDate} 23:59:59`,
  minTimestamp: convertToTimestamp(`${currentDate} 00:00:00`),
  maxTimestamp: convertToTimestamp(`${currentDate} 23:59:59`),
};
...
// 출퇴근 기록 데이터베이스 저장 함수
const saveCommuteRecord = async (record, isEnd = false) => {
  ...
  try {
    ...
    //   config.reference = record.record_id;
...
const startWork = async () => {
  ...
  console.log('=== startWork === isCommuted : ', isCommuted);
  console.log('=== startWork === commuteStorage : ', commuteStorage);
  ...
  try {
    ...
    console.log('=== startWork === savedRecord : ', savedRecord);
    console.log('=== startWork === commuteStorage : ', commuteStorage);
...
const fetchCommuteRecords = async (userId, options = {}) => {
  try {
    const {
      ...
      fetchMore = false, // 추가 데이터 가져오기 여부
