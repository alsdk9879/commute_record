...
timeRecords.value.date = '2024-12-22';
...
const startWork = () => {
	...
	if (!startLocalStorage.some(record => record.start === date && record.user_id === user.value.user_id)) {
		...
		for (let i = 0; i < originTimeRecords.value.length; i++) {
			if (originTimeRecords.value[i].date === date) {
				// 같은 날 퇴근 시간에 출근 시간 추가
				originTimeRecords.value[i].startWork = time; 
		...
		// 만약 오늘과 다른 날짜가 되었다면
		if (!dateFound) {
			originTimeRecords.value.push(newEvent); // 새로운 날짜에 대한 출근 시간 추가
		}
...
const endWork = () => {
	...
	if (!startLocalStorage.some(record => record.start === date && record.user_id === user.value.user_id)) {
		...
		// originTimeRecords.value = endLocalStorage;
		return;
	}
	...
	if (!endLocalStorage.some(record => record.end === date && record.user_id === user.value.user_id)) {
		...
		for (let i = 0; i < originTimeRecords.value.length; i++) {
			if (originTimeRecords.value[i].date === date) {
				originTimeRecords.value[i].endWork = time; // 같은 날 출근 시간에 퇴근 시간 추가
				dateFound = true; // 날짜가 존재함을 표시
				break; // 날짜가 발견되면 루프 종료
			}
		}
		...
		// 만약 오늘과 다른 날짜가 되었다면
		if (!dateFound) {
			originTimeRecords.value.push(newEvent); // 새로운 날짜에 대한 퇴근 시간 추가
		}
...
const onRecord = () => {
	...
	if (endLocalStorage.length > 0) {
		...
		for (let i = 0; i < originTimeRecords.value.length; i++) {
			const originDate = originTimeRecords.value[i].date;

			for (let j = 0; j < endLocalStorage.length; j++) {
				const endDate = endLocalStorage[j].date;

				if (originDate === endDate && originTimeRecords.value[i].user_id === endLocalStorage[j].user_id) {
					originTimeRecords.value[i].endWork = endLocalStorage[j].endWork;
					break;
				} else {
					originTimeRecords.value[i].endWork = '';
				}
			}
		}
